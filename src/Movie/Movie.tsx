...
const Hero: React.FC = () => {
    ...
    const apiurl2 = `${baseURL}/movie/${id}?api_key=${apiKey}&language=en-US`;
    const reccomendationsURL = `${baseURL}/movie/${id}/recommendations?api_key=${apiKey}&language=en-US&page=1`;

    let newItem = (data:any) => (

      <Link to = {`/movie/${data.id}`} style = {{backgroundImage: data.poster_path === null ? `url(${placeholder})` : `url(${imgURL}${data.poster_path})`}} tabIndex={0} className = 'RecommendationItem'>
  ...
  return (
    <div className = 'Movie'>
        ...
        <div style = {{backgroundImage: itemData == null ? `url(${placeholder})` : `url(${imgURL + itemData.backdrop_path})`}} className = 'MovieHero'>
          <div className='MovieHeroContent'>
            ...
            <div className = 'MovieHeroInfo'>
              <span className = 'MovieHeroTitle'>{itemData != null ? itemData.title : "..."}</span>
              <div className='MovieHeroInfo2'>
                ...
                <span className = 'MovieHeroYear'>{itemData != null ? itemData?.release_date?.substring(0,4) : "..."}</span>
                <span className = 'MovieHeroTime'>{itemData != null ? itemData?.runtime : "0"}m</span>
              </div>
        ...
        <div className = 'MovieRecommendations'>
          <span className = 'MovieContentHeader'>{fullTable?.length == 0 ? 'No Current Recommendations' : 'Recommended Movies'}</span>
