...
const App: React.FC = () => {
  ...
  const apiurl = hasSearched ? `${baseURL}/search/movie?api_key=${apiKey}&query=${searchText}&include_adult=false&release_date.lte=${formattedDate}&include_video=false&with_original_language=en&language=en-US&sort_by=${sortArray[SortBy]}&page=${currentPage > maxPage ? maxPage : currentPage}` : `${baseURL}/discover/movie?api_key=${apiKey}&include_adult=false&release_date.lte=${formattedDate}&include_video=false&with_original_language=en&language=en-US&sort_by=${sortArray[SortBy]}&page=${currentPage > maxPage ? maxPage : currentPage}`;
  ...
  let checkFontSize = () => {
    ...
    <Link to = {`/movie/${data.id}`} style = {{backgroundImage: data.poster_path === null ? `url(${placeholderIMG})` : `url(${imgURL}${data.poster_path})`}} tabIndex={0} className = 'MovieItem'>
    ...
    <div className="App">
        <div className = 'MovieHeader'>
          ...
          <div className = 'BottomHeader'>
            <input ref = {searchRef} onKeyDown={searchHandler} type = "text" placeholder='Search Movies' className = 'BottomHeaderSearch' />
        ...
        <div className = 'MovieContent'>
          ...
          {!hasSearched && <span className = 'MovieContentHeader'>Popular Movies</span>}
